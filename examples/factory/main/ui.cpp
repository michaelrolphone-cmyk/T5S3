...
static void md_render_to_spangroup(const char *text)
{
    ...
    while(*line) {
        ...
        static char span_line_buf[512];
        size_t copy_len = content_len;
        if(copy_len > sizeof(span_line_buf) - 2) copy_len = sizeof(span_line_buf) - 2;
        memcpy(span_line_buf, content, copy_len);
        span_line_buf[copy_len++] = '\n';
        span_line_buf[copy_len] = '\0';
        lv_span_set_text(sp, span_line_buf);
